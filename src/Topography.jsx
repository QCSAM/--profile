...
export const colorModeToFloat = (mode) => {
...
const Topography = ({
  ...
  useEffect(() => {
    ...
    if (!container || typeof WebGL2RenderingContext === 'undefined') return undefined
    ...
    const loop = (timestamp) => {
      ...
      const amount = uniforms.uMorphAmount.value
      const currentSpeed = uniforms.uSpeed.value
      const currentMorphSpeed = uniforms.uMorphSpeed.value

      for (let group = 0; group < 4; group += 1) {
        const array = controlArrays[group]
        const indices = controlIndices[group]
        for (let index = 0; index < 4; index += 1) {
          const phase = indices[index]
          array[index] = amount * Math.sin(time * currentSpeed * Math.sin(phase * currentMorphSpeed) + phase)
        }
      }
    ...
    const tryStart = () => {
      if (!prefersReducedMotion && isVisible && isPageVisible && frame === 0) {
        frame = requestAnimationFrame(loop)
      }
    }
    const tryStop = () => {
